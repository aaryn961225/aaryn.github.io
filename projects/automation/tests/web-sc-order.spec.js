...
test.describe('Web SC 一般訂貨流程 Demo', () => {
  ...
  test('Happy Path｜登入後可加入商品並送出訂貨', async ({ page }) => {
    ...
    await test.step('加入訂貨商品並驗證摘要', async () => {
      ...
      await expect(orderPage.summaryCount).toHaveText(String(testData.happyPathOrder.expectedItemCount));
      await expect(orderPage.summaryTotal).toHaveText(String(testData.happyPathOrder.expectedTotalQuantity));
    });
  ...
  test('Boundary｜訂貨量為 0 時可加入但總量應為 0', async ({ page }) => {
    ...
    await expect(orderPage.summaryCount).toHaveText(String(testData.boundaryOrder.expectedItemCount));
    await expect(orderPage.summaryTotal).toHaveText(String(testData.boundaryOrder.expectedTotalQuantity));
  });
});
